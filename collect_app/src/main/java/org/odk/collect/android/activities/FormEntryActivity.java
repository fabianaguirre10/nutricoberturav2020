...
import org.odk.collect.android.database.BaseDatosEngine.Entidades.BranchDevoluciones;
...
public class FormEntryActivity extends CollectAbstractActivity implements AnimationListener,
    ...
    private View createView(int event, boolean advancingPage) {
        ...
        switch (event) {
            case FormEntryController.EVENT_BEGINNING_OF_FORM:
                ...
                if(objBranchSessioot.getE_code().equals("")&& objBranchSessioot.getE_code()!="code") {
                    String saveNameUpdateu = formController.getSubmissionMetadataUpdate(objBranchSessioot.getE_code(),objBranchSessioot.getE_rutaaggregate(),objBranchSessioot.getE_nuevo());
                }else{
                    if(objBranchSessioot.getE_code()!="code"){
                        String saveNameUpdateu = formController.getSubmissionMetadataUpdate(objBranchSessioot.getE_code(), objBranchSessioot.getE_rutaaggregate(), objBranchSessioot.getE_nuevo());
                    }
                }
    ...
    private boolean cambiarestado(String codigo,String uri){
        try {
            ...
            Cursor cursor;
            cursor = new InstancesDao().getUnsenequaltInstancesCursor(codigo, "");
            boolean existefisico = false;
            boolean existebaseengine = false;
            ...
            where1 = where1 + " and  rutaaggregate ='" + objB.getE_rutaaggregate().toString() + "' ";
            where1 = where1 + "and code = '" + codigo + "'";
            Cursor cursor1 = objbuscar.SeleccionarLocal(where1);
            if (cursor1.moveToFirst()) {
                do {
                    cursor1.getString(0);
                    existebaseengine = true;
                } while (cursor.moveToNext());
            }
            ...
            if (objB.getE_nuevo().toString().equals("nuevo")) {
                ...
                if (existebaseengine == false && existefisico == true) {
                    ...
                    Objdatosingreso.put(EstructuraBD.CabecerasEngine.externalCode, codigo.toString());
                    Objdatosingreso.put(EstructuraBD.CabecerasEngine.code, codigo.toString());
                    Objdatosingreso.put(EstructuraBD.CabecerasEngine.name, "");
                    Objdatosingreso.put(EstructuraBD.CabecerasEngine.mainStreet, "");
                    Objdatosingreso.put(EstructuraBD.CabecerasEngine.neighborhood, "");
                    Objdatosingreso.put(EstructuraBD.CabecerasEngine.reference, "");
                    Objdatosingreso.put(EstructuraBD.CabecerasEngine.propietario, "");
                    Objdatosingreso.put(EstructuraBD.CabecerasEngine.uriformulario, uri);
                    Objdatosingreso.put(EstructuraBD.CabecerasEngine.idprovince, "");
                    Objdatosingreso.put(EstructuraBD.CabecerasEngine.iddistrict, "");
                ...
                if (existefisico == true) {
                    usdbh = usdbh.open();
                    ContentValues Objdatos = new ContentValues();
                    Objdatos.put(EstructuraBD.CabecerasEngine.uriformulario, uri);
                    Objdatos.put(EstructuraBD.CabecerasEngine.EstadoFormulario, objB.getE_EstadoFormulario());
                    Objdatos.put(EstructuraBD.CabecerasEngine.Colabora, objB.getE_Colabora());
                    String where2 = "code='" + codigo
                            + "'";
            ...
            case SaveToDiskTask.SAVED_AND_EXIT:
                ...
                for (BranchProducto bp: activos.getListapro()
                    ...
                    if(!bp.getE_valor().equals("0") && bp.getE_valor()!="e_valor") {
                        ...
                        for (BranchDevoluciones branchdev:formController.getListaprodev()
                        ) {
                            /*ventas y entrega producto a cambio de devoluciones*/
                            if(branchdev.getCodproducto()!=null) {
                                int codigosku = Integer.valueOf(branchdev.getCodproducto().replace("sku", ""));
                                if (branchdev.getCant_entrega() != 0 && codigosku == Integer.valueOf(bp.e_codproducto)) {
                                    c = c + branchdev.getCant_entrega();
                                    ...
                                    Objdatosop.put(EstructuraBD.CabeceraOperaciones.cantidad, branchdev.getCant_entrega());
                                ...
                                /*ventas y no entrega producto a cambio de devoluciones*/
                                if (branchdev.getCant_dev() != 0 && codigosku == Integer.valueOf(bp.e_codproducto)) {
                                    SimpleDateFormat dateFormat = new SimpleDateFormat(
                                            "yyyy-MM-dd HH:mm:ss", Locale.getDefault());
                                    Date date = new Date();

                                    ContentValues Objdatosop = new ContentValues();
                                    Objdatosop.put(EstructuraBD.CabeceraOperaciones.cantidad, branchdev.getCant_dev());
                                    Objdatosop.put(EstructuraBD.CabeceraOperaciones.codproducto, bp.getE_codproducto());
                        ...
                        /*cuando solo hace devoluciones y no ventas*/
                        for (BranchDevoluciones branchdev:formController.getListaprodev()
                        ) {
                            if(branchdev.getCodproducto()!=null) {
                                Double s = 0.0, c = 0.0;
